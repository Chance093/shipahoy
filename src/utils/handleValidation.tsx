...
function useValidation([columnHeaders, rowsOfValues]: [string[], string[][]]): [
  string[],
  string[],
] {
  type ErrorFlagDetails = Array<
    string | number | Map<string, number[]> | undefined
  >;
  const EXPECTED_COLUMN_HEADERS: string[] = [
    "FromCountry",
    "FromName",
    "FromCompany",
    "FromPhone",
  ...
  const newValidationCheckpoint = (checkpoint: string) =>
    validationCheckpoints.push(checkpoint);

  newValidationCheckpoint(
    "useValidation → Array to store error flags is initialized.",
  );
  ...
  const getErrorFlagMessage = (
    ...
    newValidationCheckpoint(
      `getErrorFlagMessage() → New error flag: '${errorFlagType}', ${errorFlagDetails}`,
    );
    const errorFlagMessage: string[] = [];
    switch (errorFlagType) {
      case "column header length":
        errorFlagMessage.push(
          "Your CSV's column headers do not match those of our template.",
        );
        return errorFlagMessage;
      case "column header value":
        const [columnNumber, invalidColumnHeader, expectedColumnHeader] =
          errorFlagDetails as [number, string, string];
        errorFlagMessage.push(
          `Column ${columnNumber}'s header is ${invalidColumnHeader}; it should be ${expectedColumnHeader}.`,
        );
        return errorFlagMessage;
      case "one or more empty values":
        ...
        for (const [key, value] of invalidIndexes) {
          ...
          errorFlagMessage.push(
            `${key} has empty values in column(s): ${columnNumbers}.`,
          );
        }
        return errorFlagMessage;
      default:
  ...
  const validateColumnHeaders = (): void => {
    ...
    newValidationCheckpoint(
      `validateColumnHeaders() → Validation for column headers done, there were ${errorFlags.length} errors flagged`,
    );
  };
  ...
  const validateRowValues = (): void => {
    ...
    errorFlags.push(...errorFlagMessage);
    newValidationCheckpoint(
      `validateRowValues() → Validation for row values done, there were ${errorFlags.length} errors flagged`,
    );
  };
  ...
  newValidationCheckpoint(
    `validateCsvContents() → Validation for column headers and row values done. Is the CSV valid: ${
      !errorFlags.length ? "Yes" : "No"
    }`,
  );
