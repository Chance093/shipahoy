...
export default function Invoices({ invoices, page, invoiceCount }: { invoices: Invoices; page: number; invoiceCount: number }) {
  // * Initialize an invoice index depending on page number
  const subractValue = page * 10 - 10;
  let invoiceIndex = invoiceCount - subractValue + 1;
            ...
            {invoices?.map((invoice) => {
              ...
              return (
                <tr key={invoice.id} className="border-b border-gray-600/50">
                  <td className="p-4 py-6">{invoiceIndex}</td>
