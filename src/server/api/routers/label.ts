  ...
  createLabel: protectedProcedure
    .input(
      z.object({
        ...
        fromZipCode: z.string().trim().regex(zipCodeRegex, { message: "Must follow phone number format" }),
        fromCity: z.string().trim(),
        fromState: z.string().trim().length(2, { message: "Must be exactly 2 characters long" }),
        fromCountry: z.string().trim(),
        fromPhoneNumber: z.string().trim().regex(phoneNumberRegex),
        toName: z.string().trim(),
        toCompanyName: z.string().trim(),
        toAddress: z.string().trim(),
        toAddress2: z.string().trim(),
        toZipCode: z.string().trim().regex(zipCodeRegex, { message: "Must follow phone number format" }),
        toCity: z.string().trim(),
        toState: z.string().trim().length(2, { message: "Must be exactly 2 characters long" }),
        toCountry: z.string().trim(),
        toPhoneNumber: z.string().trim().regex(phoneNumberRegex),
        height: z.number(),
        weight: z.number(),
        length: z.number(),
        width: z.number(),
